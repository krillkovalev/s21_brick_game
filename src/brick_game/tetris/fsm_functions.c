...
GameInfo_t init_game(FILE **file, const sketch_figure figures[]) {
    ...
    *file = fopen("record.txt", "r+"); // Открываем файл для чтения и записи
    ...
    if (fscanf(*file, "%d", &new_game.high_score) != 1) { // Считываем число из файла
    ...
    new_game.figure = create_figure(figures[new_game.number_next_figure], new_game.number_next_figure);
...
int check_rotate_position(matrix_figure shape, char **field)
{
    ...
    for (int i = shape.current_y; i < shape.current_y + new_height; i++) {
        for(int j = shape.current_x / 2; j < shape.current_x / 2 + new_width; j++){
            if((field[i][j] == 'X' && temp_rotate[i - shape.current_y][j - shape.current_x / 2] == 'X') || j >= FIELD_W || i >= FIELD_H) {
                answer_code = 1;
                break;
            }
        }
...
void remove_shift_row(int row, char **field) {  // принимает номер которую удаляем и все что выше едет вниз
...
void figure_down(GameInfo_t *game, const sketch_figure figures[]) {
    ...
    game->figure = create_figure(figures[game->number_next_figure], game->number_next_figure);
