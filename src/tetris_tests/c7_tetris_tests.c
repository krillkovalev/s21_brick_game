...
START_TEST(test_create_figure) {
  sketch_figure test_sketch = {.figure = "XXXX", .height = 2, .width = 2};
  int test_number = 1;
...
START_TEST(test_init_game) {
  ...
  ck_assert_int_eq(game.high_score, game.high_score);
...
START_TEST(test_check_left_position) {
  ...
  matrix_figure test_shape = {
      .rows = 1, .columns = 1, .current_x = 1, .current_y = 1};
...
START_TEST(test_check_down_position) {
  ...
  test_field[2][1] =
      'X';  // Устанавливаем препятствие под текущей позицией фигуры
...
START_TEST(test_part_of_ship) {
  ...
  matrix_figure test_shape = {
      .rows = 2, .columns = 2, .current_x = 1, .current_y = 1};
  test_shape.work_figure = (char **)malloc(test_shape.rows * sizeof(char *));
  test_shape.work_figure[0] = (char *)malloc(test_shape.columns * sizeof(char));
...
START_TEST(test_rotate_figure) {
  matrix_figure test_shape = {
      .rows = 2, .columns = 3, .current_x = 1, .current_y = 1};
  test_shape.work_figure = (char **)malloc(test_shape.rows * sizeof(char *));
  test_shape.work_figure[0] = (char *)malloc(test_shape.columns * sizeof(char));
  test_shape.work_figure[1] = (char *)malloc(test_shape.columns * sizeof(char));
  test_shape.work_figure[0][0] = 'X';
  ...
  ck_assert_int_eq(
      rotated_shape.rows,
      3);  // Проверяем, что количество строк и столбцов поменялись местами
  ck_assert_int_eq(rotated_shape.columns, 2);

  ck_assert_int_eq(rotated_shape.work_figure[0][0],
                   ' ');  // Проверяем, что фигура была правильно повернута
...
START_TEST(test_check_full_row) {
  ...
  ck_assert_int_eq(removed_rows,
                   1);  // Проверяем, что одна полная строка была удалена
...
START_TEST(test_figure_down) {
  ...
  ck_assert_int_eq(game.score,
                   initial_score + count_score(check_full_row(game.field)));
...
START_TEST(test_check_left_position_on_edge) {
  // заменить на функцию создания поля если есть
  char **field = malloc(FIELD_H * sizeof(char *));
  for (int i = 0; i < FIELD_H; i++) {
    field[i] = calloc(FIELD_W, sizeof(char));
    // В каждой строке первый символ устанавливается в 'X', что символизирует
    // левую границу игрового поля.
    field[i][0] = 'X';
  }
  // заменить функцией фигуры если есть
  matrix_figure shape;
  shape.rows = 1;
  shape.columns = 1;
  shape.current_x = 0;  // Положение фигуры у левого края в нулевом столбце
  shape.current_y = 5;
  shape.work_figure = malloc(shape.rows * sizeof(char *));
  for (int i = 0; i < shape.rows; i++) {
    shape.work_figure[i] = malloc(shape.columns * sizeof(char));
    shape.work_figure[i][0] = 'X';
  }

  // Вызов функции проверки
  int result = check_left_position(shape, field);
...
START_TEST(test_check_left_position_movable) {
  // фигура может свободно переместиться влево
  char **field = malloc(FIELD_H * sizeof(char *));
  for (int i = 0; i < FIELD_H; i++) {
    field[i] = calloc(FIELD_W, sizeof(char));
  }
  ...
  shape.columns = 1;
  shape.current_x =
      2;  // Положение фигуры не у края, есть пространство для движения
  shape.current_y = 5;
  shape.work_figure = malloc(shape.rows * sizeof(char *));
  for (int i = 0; i < shape.rows; i++) {
    shape.work_figure[i] = malloc(shape.columns * sizeof(char));
...
START_TEST(test_check_left_position_next_cell_occupied) {
  // слева от фигуры находится препятствие 'X'
  char **field = malloc(FIELD_H * sizeof(char *));
  for (int i = 0; i < FIELD_H; i++) {
    field[i] = calloc(FIELD_W, sizeof(char));
  }
  ...
  shape.rows = 1;
  shape.columns = 1;
  shape.current_x = 4;  // Фигура не у края и слева от нее есть клетка
  shape.current_y = 5;
  shape.work_figure = malloc(shape.rows * sizeof(char *));
  for (int i = 0; i < shape.rows; i++) {
    shape.work_figure[i] = malloc(shape.columns * sizeof(char));
  ...
  // препятствие слева от фигуры
  field[shape.current_y][shape.current_x / 2 - 1] = 'X';
...
START_TEST(test_check_right_position_on_edge) {
  char **field = malloc(FIELD_H * sizeof(char *));
  for (int i = 0; i < FIELD_H; i++) {
    field[i] = calloc(FIELD_W, sizeof(char));
    // Задаем правую границу поля 'X'
    field[i][FIELD_W - 1] = 'X';
  }
  ...
  shape.rows = 1;
  shape.columns = 1;
  // Положение фигуры у правого края (учитывая ширину)
  shape.current_x = (FIELD_W - 1) * 2;
  shape.current_y = 5;
  shape.work_figure = malloc(shape.rows * sizeof(char *));
  for (int i = 0; i < shape.rows; i++) {
    shape.work_figure[i] = malloc(shape.columns * sizeof(char));
  ...
  int result = check_right_position(shape, field);
...
START_TEST(test_check_right_position_movable) {
  // фигура может свободно переместиться вправо
  char **field = malloc(FIELD_H * sizeof(char *));
  for (int i = 0; i < FIELD_H; i++) {
    field[i] = calloc(FIELD_W, sizeof(char));
  }
  ...
  shape.columns = 1;
  shape.current_x =
      2;  // Положение фигуры не у края, есть пространство для движения вправо
  shape.current_y = 5;
  shape.work_figure = malloc(shape.rows * sizeof(char *));
  for (int i = 0; i < shape.rows; i++) {
    shape.work_figure[i] = malloc(shape.columns * sizeof(char));
...
START_TEST(test_check_right_position_next_cell_occupied) {
  // справа от фигуры находится препятствие 'X'
  char **field = malloc(FIELD_H * sizeof(char *));
  for (int i = 0; i < FIELD_H; i++) {
    field[i] = calloc(FIELD_W, sizeof(char));
  }
  ...
  shape.rows = 1;
  shape.columns = 1;
  shape.current_x = 2;  // Фигура не у края и справа от нее есть клетка
  shape.current_y = 5;
  shape.work_figure = malloc(shape.rows * sizeof(char *));
  for (int i = 0; i < shape.rows; i++) {
    shape.work_figure[i] = malloc(shape.columns * sizeof(char));
  ...
  // !
  field[shape.current_y][shape.current_x / 2 + 1] =
      'X';  // Ставим препятствие справа от фигуры
...
START_TEST(test_check_down_position_on_bottom_edge) {
  char **field = malloc(FIELD_H * sizeof(char *));
  for (int i = 0; i < FIELD_H; i++) {
    field[i] = calloc(FIELD_W, sizeof(char));
  }
  ...
  shape.columns = 2;
  shape.current_x = 2;
  shape.current_y = FIELD_H - shape.rows;  // Положение фигуры у нижнего края
  shape.work_figure = malloc(shape.rows * sizeof(char *));
  for (int i = 0; i < shape.rows; i++) {
    shape.work_figure[i] = malloc(shape.columns * sizeof(char));
    for (int j = 0; j < shape.columns; j++) {
  ...
  int result = check_down_position(shape, field);
...
START_TEST(test_check_down_position_with_obstacle_below) {
  // под фигурой находится препятствие 'X'
  char **field = malloc(FIELD_H * sizeof(char *));
  for (int i = 0; i < FIELD_H; i++) {
    field[i] = calloc(FIELD_W, sizeof(char));
  }
  ...
  shape.columns = 2;
  shape.current_x =
      4;  // Фигура не у края, поэтому под нею может быть препятствие
  shape.current_y =
      3;  // Указываем, что фигура находится выше, чем одна строка от низа
  shape.work_figure = malloc(shape.rows * sizeof(char *));
  for (int i = 0; i < shape.rows; i++) {
    shape.work_figure[i] = malloc(shape.columns * sizeof(char));
    for (int j = 0; j < shape.columns; j++) {
      shape.work_figure[i][j] = 'X';  // Заполняем фигуру
    }
  }

  field[shape.current_y + 1][shape.current_x / 2] = 'X';
...
Suite *tetris_suite(void) {
  ...
  tcase_add_test(tc, test_check_left_position_on_edge);
  tcase_add_test(tc, test_check_left_position_movable);
  tcase_add_test(tc, test_check_left_position_next_cell_occupied);

  tcase_add_test(tc, test_check_right_position_on_edge);
  tcase_add_test(tc, test_check_right_position_movable);
  tcase_add_test(tc, test_check_right_position_next_cell_occupied);

  tcase_add_test(tc, test_check_down_position_on_bottom_edge);
  tcase_add_test(tc, test_check_down_position_with_obstacle_below);
